...
include('../../admin/config/config.php');
	...
	if(isset($_GET['cong'])){
		...
		foreach($_SESSION['cart'] as $cart_item){
            $sql_sl= 'SELECT soluong FROM tbl_sanpham WHERE tbl_sanpham.tensanpham = "'.$cart_item['tensanpham'].'"';
            $query_sl= mysqli_fetch_array(mysqli_query($mysqli,$sql_sl));
			if($cart_item['id']!=$id){
				...
				if($cart_item['soluong']<= $query_sl[0]-1 ){	
    ...
    if($row){
        ...
        if(isset($_SESSION['cart'])){
            ...
            foreach($_SESSION['cart'] as $cart_item){
                //neu du lieu trung
                if($cart_item['id']==$id){
                    $product[]= array('tensanpham'=>$cart_item['tensanpham'],'id'=>$cart_item['id'],'soluong'=>$soluong,'giamgia'=>$cart_item['giamgia'],'hinhanh'=>$cart_item['hinhanh'],'masp'=>$cart_item['masp']);
                    $found = true;
                }else{
